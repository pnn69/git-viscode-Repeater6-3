...
static bool Oled = false;   // Oled display connected
static bool keyIO = false;  // IO extener keys front connected
...
void i2cscanner(void) {
  ...
  for (i = 3; i < 0x78; i++) {
    ...
    if (espRc == 0) {
      printf(" %.2x", i);
      if (i == 0x3c) Front = true;
      if (i == 0x24) keyIO = true;
    } else {
      printf(" --");
    }
    i2c_cmd_link_delete(cmd);
  ...
  printf("\n\n");
  if (Oled == true) ESP_LOGI(TAG, "Display found!");
  if (keyIO == true) ESP_LOGI(TAG, "Key extender found!");
}
