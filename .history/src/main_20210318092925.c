...
//**************************************************************************************************
		case modeHumidifier:
				...
				if (secstamp + OneSec <= xTaskGetTickCount()) {
					...
					//ESP_LOGI(TAG,"T0:%d[%3d,%03d,%03d] T1:%d[%3d,%03d,%03d] T2:%d[%3d,%03d,%03d] Approx:%d Sec", T0,TF0,R0,TF0-R0,T1, TF1,R1,TF1-R1,T2, TF2,R2,TF2-R2, aproxtimer);
...
//**************************************************************************************************
	  		case modeFanPumpController:
	  			if(xSemaphoreTake(xSemaphoreSTR,pdTRUE)){
					if(Fan.speedfan > 0){
						tmp = Fan.speedfan;
						if(tmp > Fan.speedmaximalfan) tmp = Fan.speedmaximalfan; //man fanspeed
						if(tmp < Fan.speedminimalfan) tmp = Fan.speedminimalfan; //min fanspeed
					} else {
						tmp = 0; //fan off
					}
					dim1 =(int)(tmp*10);
				...
				if(xSemaphoreTake(xSemaphoreVIN,10 == pdTRUE)){
					if(voltageFAN > 2 || PressLowAlarm == true || PressHighAlarm ==  true){
						dim2 = 0;
					} else {
						dim2 = 1000;
					}
...
//Debug info
				if (secstamp + OneSec <= xTaskGetTickCount()) {
					...
					if(xSemaphoreTake(xSemaphoreNTC,pdTRUE)){
						if(xSemaphoreTake(xSemaphoreSTR,pdTRUE)){
							//ESP_LOGI(TAG, "Tout:%03.02fC Fan:%03.0f%% Tint:%03.02fC Tset:%02.0fC DeltaT:%04.02fC Pump:%03.0f%% C:%s",NTC[1],Fan.speedfan,NTC[0],Fan.Tout,NTC[0]-NTC[1],Pump.speedpump,(CompressorOn == true) ? "ON":"OFF");
							//ESP_LOGI(TAG, " Tout:%03.02fC Fan:%06.02f%% ErrFan:%06.02f%%   Tin:%03.02fC TDelta:%05.02fC Pump:%06.02f%% ErrPump:%06.02f%%    C:%s",NTC[ToutPos],Fan.speedfan,FanError,NTC[TinPos],NTC[TinPos]-NTC[ToutPos],Pump.speedpump,PumpError,(CompressorOn == true) ? "ON":"OFF");
							//ESP_LOGI(TAG, " Tout:%03.02fC Fan:%06.02f%% ErrFan:%06.02f%%   Tin:%03.02fC TDelta:%05.02fC Pump:%06.02f%% ErrPump:%06.02f%%  %s I:%06.02f",NTC[ToutPos],Fan.speedfan,FanError,NTC[TinPos],NTC[TinPos] -Fan.Tout,Pump.speedpump,PumpError,(CompressorOn == true) ? "ON":"OFF", Fan.iErrorfan);
