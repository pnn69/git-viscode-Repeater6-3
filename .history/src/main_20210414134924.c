        ...
        if (ch != 0xFF) {
          ...
          if (ch == '2') {
            ...
            for (int tel = 0; tel < 15; tel++) {
              ESP_LOGI(TAG, "P%02d RAW %04d %.3fV", tel, ADC[tel],
                       ((float)(ADC[tel] - ADC[OvREF]) * 2.2 / 2048));
            }
            ESP_LOGI(TAG, "FAN   %03.02fV ", voltageFAN);
            ESP_LOGI(TAG, "HEAT  %03.02fV ", voltageHEAT);
            ESP_LOGI(TAG, "RH    %03.02fV\r\n", voltageRH);
            ESP_LOGI(TAG, "T1   %d Raw", ADC[ANL2adr]);
            ESP_LOGI(TAG, "T2   %d Raw" ,ADC[ANL3adr]);
            ESP_LOGI(TAG, "T3   %d Raw",ADC[ANL4adr]);
            ESP_LOGI(TAG, "T1   %03.02fC ", new_ntc_sample(ADC[ANL2adr]));
            ESP_LOGI(TAG, "T2   %03.02fC ", new_ntc_sample(ADC[ANL3adr]));
            ESP_LOGI(TAG, "T3   %03.02fC ", new_ntc_sample(ADC[ANL4adr]));
          }
