...
void IRAM_ATTR timer_isr_1(void *para) {
    ...
    case 3:
        if (executeChnn[1] < PULSE_OFF) {
            next =1000 + executeChnn[1] - executeChnn[0];
            loopcnt = 4;
        }     
        break;      
    case 4:
        if (executeChnn[2] < PULSE_OFF) {
            next = 1000 + executeChnn[2] - executeChnn[1];
            loopcnt = 5;
        }
        break;
    default:
