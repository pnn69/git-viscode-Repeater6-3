  ...
  while (1) {
    switch (mode) {
      ...
      case modeFanAuxBoxRetro:
        ...
        if (ch != 0xFF) {
          ...
          if (ch == '2') {
            ...
            ESP_LOGI(TAG, "P7 RAW %04d  %0.2fV", ADC[6],
                     ADC[6] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P8 RAW %04d  %0.2fV", ADC[7],
                     ADC[7] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P8 RAW %04d  %0.2fV", ADC[8],
                     ADC[8] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P8 RAW %04d  %0.2fV", ADC[9],
                     ADC[9] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P9 RAW %04d  %0.2fV", ADC[10],
                     ADC[10] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P10 RAW %04d  %0.2fV", ADC[11],
                     ADC[11] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P11 RAW %04d  %0.2fV", ADC[12],
                     ADC[12] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P12 RAW %04d  %0.2fV", ADC[13],
                     ADC[13] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P13 RAW %04d  %0.2fV", ADC[14],
                     ADC[14] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P14 RAW %04d  %0.2fV", ADC[15],
                     ADC[15] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);
            ESP_LOGI(TAG, "P15 RAW %04d  %0.2fV", ADC[16],
                     ADC[16] * NVMsystem.NVMVgain * NVMsystem.NVMVgain / 1000);

            ESP_LOGI(TAG, "FAN   %03.02fV ", voltageFAN);
            int samp = AdcSampRaw();
            ESP_LOGI(TAG, "ADC RAW   %03d ", samp);
          }
      ...
      case modeFanPumpController:
        ...
        if (PressLow < 0.5 && PressLow != NAN) {
          PressLowAlarm = true;
        } else {
          PressLowAlarm = false;
        }
        if (PressHigh < 0.5 && PressHigh != NAN) {
          PressHighAlarm = true;
        } else {
      ...
      case modeFanPumpBoxRetro:
        ...
        if (ch != 0xFF) {
          ...
          if (ch == '2') {
            ...
            ESP_LOGI(TAG, "PH RAW %04d  %0.2fV", ADC[7],
                     ADC[7] * NVMsystem.NVMVgain * VgainRJ12 / 1000);
            ESP_LOGI(TAG, "FAN   %03.05fV ", voltageFAN);
            ESP_LOGI(TAG, "RF   %03.02fV ", voltageRH);
