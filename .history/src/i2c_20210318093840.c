  ...
  for (;;) {
	  if (adcstamp + pdMS_TO_TICKS(1) <= xTaskGetTickCount()) {
		  ...
		  if(xSemaphoreTake(xSemaphoreVIN,10 == pdTRUE)){
			  //voltageFAN = (ReadVoltage(AdcFanRaw())*VgainFan);//correction factor = actual / measured
              //voltageFAN = (ReadVoltage(AdcFanRaw())*1.5559);//correction factor = actual / measured
              voltageFAN = (ReadVoltage(AdcFanRaw()*4.5));//correction factor = actual / measured
		  ...
		  switch(adc_select){
		  	 ...
		  	 case 3:
		  		 ...
		  		 //ESP_LOGI(TAG,"P4 Vin:%.2fV %s",adcsamp,(DayNight==1) ? "Day":"Night");
		  		 //ESP_LOGI(TAG, "P4 Vin:%.2fV ",adcsamp);
		  		 break;
		  	 case 4:
		  		 ADC[4] = (int)adcsamp;
		  	 ...
		  	 case 7:
		  		 ADC[7] = (int)adcsamp;
		  		 if(xSemaphoreTake(xSemaphoreVIN,10 == pdTRUE)){
		  			 voltageHEAT = ADCmV[adc_select]*VgainRJ12/1000; //result in mV;
		  			 xSemaphoreGive( xSemaphoreVIN );
		  		 }
		  		 break;
		  }
		...
		//ESP_LOGI(TAG, "ADC Raw %4d  %4d %4d %4d %4d %4d",ADC[0],ADC[1],ADC[2],ADC[3],ADC[4],ADC[5]);
	  ...
	  if(dispayOnOff){
		  if((key0 || key1 || key2)){
			  ...
			  if((key0 && key1)||(key0 && key2)||(key0 && key2)||(key1 && key2)){
				  calibrate = true;
				  ESP_LOGI(TAG,"Re calibrating keys!");
				  IOstatus ^= (-!1 ^ IOstatus) & (1UL << buzzer);  	// Changing the nth bit to x
			  ...
			  //				ESP_LOGI(TAG,"T0:%d[%3d,%03d,%03d] T1:%d[%3d,%03d,%03d] T2:%d[%3d,%03d,%03d] Approx:%d", T0,TF0,R0,TF0-R0,T1, TF1,R1,TF1-R1,T2, TF2,R2,TF2-R2, aproxtimer);
			if(key0){
				ESP_LOGI(TAG, "Key UP");
				if(mode == modeHumidifier) 				LCD_menu_1(UP);
			...
			if(key1){
				...
				if(mode == modeHumidifier) 				LCD_menu_1(DOWN);
			...
			if(key2){
				...
				if(mode == modeHumidifier) 				LCD_menu_1(ENTER);
			...
			if (menustamp + pdMS_TO_TICKS(100) < xTaskGetTickCount()) {
			  ...
			  if(mode == modeHumidifier) 				LCD_menu_1(0);
