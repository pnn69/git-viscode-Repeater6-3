...
char HedyInitialise[]  		= {9,0x01,0x10,0x00,0x06,0x00,0x01,0x02,0x17,0x70};
char HedyKeyPad[] 	 		= {6,0x01,0x06,0x00,0x00,0x00,0x01};	//p00.00 Auto switchmode
char HedyDspMode[]    		= {6,0x01,0x06,0x00,0x01,0x00,0x01};	//p00.01 Dsp mode
char HedyCntrMode[]    		= {6,0x01,0x06,0x00,0x02,0x00,0x02};	//p00.03 Control mode
char HedySource[] 	 		= {6,0x01,0x06,0x00,0x03,0x00,0x05};	//p00.04 Auto source control
char HedyRunOn[]       		= {6,0x01,0x06,0x01,0x3F,0x00,0x01};	//p03.20
char HedyControlOff[]      	= {6,0x01,0x06,0x01,0x46,0x00,0x00};	//p03.27 Control word
char HedyControlOn[]       	= {6,0x01,0x06,0x01,0x46,0x00,0x02};	//p03.27 Control word
char HedyControlEnable[]  	= {6,0x01,0x06,0x01,0x47,0x00,0x01};	//p03.28 Control word enable
char HedyRunOff[]      		= {6,0x01,0x06,0x01,0x3F,0x00,0x00};
char HedyRpm0[]        		= {6,0x01,0x06,0x00,0x05,0x00,0x00};	//p00.06
char HedyRpmMax[]      		= {6,0x01,0x06,0x00,0x06,0x13,0x88}; 	//p00.07 = 50000 (50Hz)
...
char HedypwrAct[]      		= {6,0x01,0x03,0x02,0x08,0x00,0x01}; 	//p05.21 = 0.01kWh
...
//char HedyAutotuneFan[]    		= {6,0x01,0x06,0x00,0x10,0x00,0x00};	//p00.17 = 0

char HedyMotorVoltageFan[]	= {6,0x01,0x06,0x00,0x0C,0x00,0xE6};	//p00.13 = 230V
char HedyMotorCurrentFan[]	= {6,0x01,0x06,0x00,0x0D,0x00,0x24};	//p00.14 = 3.6A
char HedyMotorSpeedFan[]  	= {6,0x01,0x06,0x00,0x0E,0x13,0x88};	//p00.15 = 50Hz
char HedyAutotuneFan[]    	= {6,0x01,0x06,0x00,0x10,0x00,0x00};	//p00.17 = 2
char HedyMVCMFan[]        	= {6,0x01,0x06,0x00,0x13,0x00,0x03};	//p00.20 = 3 single phase with cap
char HedySFFan[]          	= {6,0x01,0x06,0x00,0x14,0x00,0x0B};	//p00.21 = 11
char HedyOutputPhaseFan[] 	= {6,0x01,0x06,0x04,0xB9,0x00,0x01};	//p12.10 = 1

char HedyMotorVoltagePump[]	= {6,0x01,0x06,0x00,0x0C,0x00,0xE6};	//p00.13 = 230V
char HedyMotorCurrentPump[]	= {6,0x01,0x06,0x00,0x0D,0x00,0x3B};	//p00.14 = 5.9A
char HedyMotorSpeedPump[]  	= {6,0x01,0x06,0x00,0x0E,0x13,0x88};	//p00.15 = 50Hz
char HedyAutotunePump[]    	= {6,0x01,0x06,0x00,0x10,0x00,0x00};	//p00.17 = 0
char HedyMVCMPump[]        	= {6,0x01,0x06,0x00,0x13,0x00,0x03};	//p00.20 = 0   //4 single phase with cap
char HedySFPump[]          	= {6,0x01,0x06,0x00,0x14,0x00,0x0D};	//p00.21 = 13//   11
char HedyOutputPhasePump[] 	= {6,0x01,0x06,0x04,0xB9,0x00,0x01};	//p12.10 = 1

//char HedyMotorVoltagePump[]	= {6,0x01,0x06,0x00,0x0C,0x00,0xE6};	//p00.13 = 230V
//char HedyMotorCurrentPump[]	= {6,0x01,0x06,0x00,0x0D,0x00,0x24};	//p00.14 = 3.6A
//char HedyMotorSpeedPump[]  	= {6,0x01,0x06,0x00,0x0E,0x13,0x88};	//p00.15 = 50Hz
//char HedyAutotunePump[]    	= {6,0x01,0x06,0x00,0x10,0x00,0x00};	//p00.17 = 2
//char HedyMVCMPump[]        	= {6,0x01,0x06,0x00,0x13,0x00,0x00};	//p00.20 = 0 single phase with cap
//char HedySFPump[]          	= {6,0x01,0x06,0x00,0x14,0x00,0x11};	//p00.21 = 11
//char HedyOutputPhasePump[] 	= {6,0x01,0x06,0x04,0xB9,0x00,0x00};	//p12.10 = 1
...
bool initHedyFan(int uart,int port){
    ...
    uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyModelVersion));
	...
	if (InCrc16((char *)readb, f)) {
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyInitialise));
		uart_flush_input(uart);
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorVoltageFan));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorCurrentFan));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorSpeedFan));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyAutotuneFan));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyOutputPhaseFan));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyControlEnable));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
...
bool initHedyPump(int uart,int port){	bool ret = false;
	...
	if (InCrc16((char *)readb, f)) {
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorVoltagePump));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorCurrentPump));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyMotorSpeedPump));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);

		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyAutotunePump));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyOutputPhasePump));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
...
bool FG6485_Temp_RH_task(int uart,int port) {
  ...
  if (InCrc16((char *)readb, f)) {
	  hum_FG6485[port] = ((float)(readb[3] << 8 | readb[4]))/10;			// extract humidity
	  tmp_FG6485[port] = ((float)(readb[5] << 8 | readb[6]))/10;			// extract temperature
...
void PumpFanPid(void){
	...
	if(xSemaphoreTake(xSemaphoreSTR,10 == pdTRUE)){
		...
		PID_Output = (Fan.Pfan/10*Error) + (Fan.Ifan*Fan.iErrorfan) + (Fan.Dfan*Fan.dEerrorfan) + Errorp*5;
		...
		xSemaphoreGive(xSemaphoreSTR);
		//ESP_LOGI(TAG, "P:%06.3f    I:%06.3f     D:%06.3f        error:%5.2f", (Fan.Pfan*Error),(Fan.Ifan*Fan.iErrorfan),(Fan.Dfan*Fan.dEerrorfan),Error);
	}
}
...
void PumpPumpPid(void){
	...
	if(xSemaphoreTake(xSemaphoreSTR,10 == pdTRUE)){
		...
		PID_Output = ((Pump.Ppump/10)*Error) + ((Pump.Ipump)*Pump.iErrorpump) + ((Pump.Dpump/10)*Pump.dEerrorpump);
		if(PID_Output < 0)
			PID_Output = 0;
		...
		xSemaphoreGive(xSemaphoreSTR);
		//ESP_LOGI(TAG, "Tint:%04.2fC Pump:%04.2f P:%06.3f I:%06.3f D:%06.3f error:%5.2f",NTC[TinPos], Pump.speedpump, ((Pump.Ppump/10)*Error),((Pump.Ipump)*Pump.iErrorpump),((Pump.Dpump/10)*Pump.dEerrorpump),Error);
	}
}
...
void HumiPid(void){
	...
	if(xSemaphoreTake(xSemaphoreNTC,10 == pdTRUE)){
		if( isnan(hum_FG6485[1])){
			...
			if(hum_FG6485[1] == 0 && tmp_FG6485[1] == 0){
				ESP_LOGE("RS485", "FG6485 Temp=0 and RH=0 probably still intialising!!");
				xSemaphoreGive(xSemaphoreNTC);
				return;
			}
	...
	if(xSemaphoreTake(xSemaphoreSTR,10 == pdTRUE)){
		...
		if(PID_Output < 0)
			PID_Output = 0 ;
		//ESP_LOGI(TAG, "P:%02.2f I:%02.2f PID:%02.2f RH_set:%02.2f RS_act:%02.1f",Kabel400.P*Error,Kabel400.I*Error_Integral,PID_Output, RH_set,hum_FG6485);
...
void humidifierKabel400(int uart,int port) {
	...
	if(xSemaphoreTake(xSemaphoreSTR,10 == pdTRUE)){
		...
		if(Kabel400.offTime >= 60*60*3){
			...
			Kabel400.extventOn = false; 				//close external valve
			Kabel400.floatsensorError = 0;				//reset water low timeout
			if(Kabel400.offTime >= 60*60*3 + 60 * 5){	//close drain afer 60 sec *60 * 3(3 hours)  + 5*60 (and 5 min)
		...
		*/
		if(Kabel400.pumpOn == false){
			if(Kabel400.circulateTimer*60 > Kabel400.circulateTime )Kabel400.circulateTime++;
			else{
		...
		writb[4] = Kabel400.pumpOn*pump + Kabel400.drainOn*drain +Kabel400.extventOn*extvent + Kabel400.purifyvalveOn*purifyvalve;
		...
		if (readb[0] == 0x08 && readb[1] == 0x51) {
			...
			//ESP_LOGI(TAG, "Fan speed %02f RH_set:%02.1f RS_act:%02.1f",Kabel400.fanspeed,Kabel400.RH_set,hum_FG6485 );
			Kabel400.onTime++;
		}else{
			Kabel400.connected = false;
			Kabel400.floatsensor = 0XFF;			// extract aqua level 0xFF means full 0xB4 means empty
		}
		xSemaphoreGive(xSemaphoreSTR);
	}
}
...
void humidifierRetro(int uart,int port) {
	if(HedyFan ==  false){
		...
		uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedySoftVersion));
		uart_flush_input(uart);
		f = uart_read_bytes(uart,(uint8_t *)readb, 7, 10);
		if (InCrc16((char *)readb, f)) {
		    int Hedy = readb[3] << 8 | readb[4];
	...
	if(HedyFan ==  true){
		...
		//ESP_LOGI(TAG, "Hedy inverter status RPMset:%.2f RMPact:%.2f VCC:%.2f I:%.2f P:%.2f ",(float)rpm/100,(float)HedyRPM/100,(float)HedyVCC,(float)HedyCUR/100,(float)HedyVCC*(float)HedyCUR/100);
	}
}
...
#endif
			if(Fan.speedfan != 0){
				if(Fan.speedfan > Fan.speedmaximalfan)Fan.speedfan = Fan.speedmaximalfan;
				if(Fan.speedfan < Fan.speedminimalfan)Fan.speedfan = Fan.speedminimalfan;
			}
			//if(CompressorOn == true && Fan.speedfan < Fan.speedminimalfan)Fan.speedfan = Fan.speedminimalfan;
			if(Fan.speedfan > 0){
				uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyControlOn));
				uart_flush_input(uart);
				f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
			 } else {
				  uart_write_bytes_RS485(uart,port,writb, prepair_buf(writb, HedyControlOff));
				uart_flush_input(uart);
				f = uart_read_bytes(uart,(uint8_t *)readb, 8, 10);
			}

			//skip 47Hz to 49Hz
			if(Fan.speedfan < 4700/50 || Fan.speedfan > 4900/50){ //Bandfilter fan
				hedy_set_rpm(uart,port,(uint16_t)Fan.speedfan*50);//convert percentage to rmp 100 = 1Hz
				uart_flush_input(uart);
				f = uart_read_bytes(uart,(uint8_t *)readb, 7, 10);
			}
			xSemaphoreGive(xSemaphoreSTR);
...
*/
			//ESP_LOGI(TAG, "HedyFan  set:%04dRPM act:%04dRPM VCC:%03dV I:%04dmA",rpm,HedyRPM,HedyVCC,HedyCUR*100);
...
void PumpPumpInvereter(int uart,int port) {
	...
	if(HedyPump ==  false){
		...
		if(xSemaphoreTake(xSemaphoreSTR,20 == pdTRUE)){
			...
			if(CompressorOn == true   && CompressorOnTime < 60*4){ //short time for speedup testine
				Pump.speedpump = 40;
				Pump.iErrorpump = 0;
			...
			hedy_set_rpm(uart,port,Pump.speedpump*50); //Correction for inverter 100% -> 50.000
			uart_flush_input(uart);
			f = uart_read_bytes(uart,(uint8_t *)readb, 7, 10);
			xSemaphoreGive(xSemaphoreSTR);
			...
			//ESP_LOGI(TAG, "HedyPump set:%04dRPM act:%04dRPM VCC:%03dV I:%04dmA",rpm,HedyRPM,HedyVCC,HedyCUR*100);
*/
		}
	}
}
