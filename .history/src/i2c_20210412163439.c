...
int ADCadress[16][2] = {{ANL1adr, 0}, {ANL2adr, 0}, {ANL3adr, 0}, {ANL4adr, 0},
                        {ANL5adr, 0}, {ANL6adr, 0}, {ANL7adr, 0}, {ANL8adr, 0},
                        {FANadr, 0},  {TEMPadr, 0}, {RHadr, 0},   {RHadr, 0},
                        {RHadr, 0},   {RHadr, 0}};
  ...
  for (;;) {
    ...
    if (adcstamp + pdMS_TO_TICKS(1) <= xTaskGetTickCount()) {
      ...
      if (xSemaphoreTake(xSemaphoreVIN, 10 == pdTRUE)) {
        ...
        // voltageFAN = (ReadVoltage(AdcFanRaw()));// * VgainFan);  //
        // correction factor = actual / measured voltageFAN = (AdcFanRaw());// *
