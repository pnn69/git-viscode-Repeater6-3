  ...
  if (gpio_num == ZerroCrossPin) {
    ...
    if (esp_timer_get_time() - samp > 9000) {
      ...
      if (enableOutput == false) {
        ...
        if (dim1 != 0)
          executeChnn[0] =
              DimMap(dim1, 1000, 0, offsetcorrection, 1000 - treshold) + skew;
        else {
          gpio_set_level((gpio_num_t)AC_pin1, 0);
        ...
        if (dim2 != 0)
          executeChnn[1] =
              DimMap(dim2, 1000, 0, offsetcorrection, 1000 - treshold + skew);
        else {
          gpio_set_level((gpio_num_t)AC_pin2, 0);
        ...
        if (dim3 != 0)
          executeChnn[2] =
              DimMap(dim3, 1000, 0, offsetcorrection, 1000 - treshold + skew);
        else {
          gpio_set_level((gpio_num_t)AC_pin3, 0);
