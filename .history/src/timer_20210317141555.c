...
static DRAM_ATTR volatile int loopcnt;volatile bool enableOutput = false;
...
// 578,609,639,669,698,727,754,781,806,830,853,875,895,913,930,945,959,970,980,988,994,998,1000 };
	...
	//static int16_t tbl[len] = {0,5,14,29,47,71,98,129,164,202,242,286,331,378,426,475,524,573,621,668,714,757,798,836,870,902,929,952,971,985,995,1000 };
    ...
    if (gpio_num == ZerroCrossPin) {
        ...
        if (esp_timer_get_time() - samp > 9000) {
            ...
            if (enableOutput==false) {
                ...
                if (dim1 != 0)
                    executeChnn[0] = DimMap(dim1, 1000, 0, offsetcorrection, 1000 - treshold) + skew;
                else {
                    gpio_set_level((gpio_num_t)AC_pin1, 0);
                ...
                if (dim2 != 0 )
                    executeChnn[1] = DimMap(dim2, 1000, 0, offsetcorrection, 1000 - treshold + skew);
                else {
                    gpio_set_level((gpio_num_t)AC_pin2, 0);
                ...
                if (dim3 != 0)
                    executeChnn[2] = DimMap(dim3, 1000, 0, offsetcorrection, 1000 - treshold + skew);
                else {
                    gpio_set_level((gpio_num_t)AC_pin3, 0);
                ...
                //for (int ch = 0; ch < nchannel - 1; ch++) { // bubble sort by time, PULSE_OFF is always biggest
                for (int ch = 0; ch < nchannel ; ch++) { // bubble sort by time, PULSE_OFF is always biggest                
...
void IRAM_ATTR timer_isr_1(void *para) {
    ...
    gpio_set_level((gpio_num_t)channel[loopcnt], 1);
    timer_get_counter_value(TIMER_GROUP_0, TIMER_0, &timerstamp);
    //Delay 10 us
    wait =esp_timer_get_time();
    while(wait + 10 >  esp_timer_get_time());

    if (loopcnt < 3 && executeChnn[loopcnt] > offsetcorrection + skew)
        gpio_set_level((gpio_num_t)channel[loopcnt], 0);
    ...
    case 1:
        ...
        break;
    case 2:
        next = 1000 - (executeChnn[2] - executeChnn[0]);
        loopcnt = 3;
        break;
    case 3:
        TIMERG0.hw_timer[0].config.alarm_en = 0;
...
int init_timer(uint16_t set) {
    ...
    // timer_config_t config = {.alarm_en = TIMER_ALARM_EN, .counter_en = false, .intr_type = TIMER_INTR_LEVEL, .counter_dir = TIMER_COUNT_UP, .auto_reload = TIMER_AUTORELOAD_EN, .divider = 80};
    if (div != 0) {
