...
int ADCadress[16][2] = {{0x10, 0}, {0x28, 0}, {0x30, 0}, {0x01, 0},
                        {0x02, 0}, {0x06, 0}, {0x05, 0}, {0x07, 0},
                        {0x08, 0}, {0x00, 0}, {0x18, 0}};
  ...
  for (;;) {
    ...
    if (adcstamp + pdMS_TO_TICKS(1) <= xTaskGetTickCount()) {
      ...
      if (xSemaphoreTake(xSemaphoreVIN, 10 == pdTRUE)) {
        voltageFAN = map(AdcFanRaw(),47,1146,0,12);
        //voltageFAN = (ReadVoltage(AdcFanRaw()));// * VgainFan);  // correction factor = actual / measured
        //voltageFAN = (AdcFanRaw());// * VgainFan);  // correction factor = actual / measured
      ...
      IOstatus = i2c_read_device(I2C0_EXPANDER_ADDRESS_MUX);
      IOstatus = IOstatus & 0x3F;
      IOstatus = IOstatus | (adc_select);  // set mux to selected channel
