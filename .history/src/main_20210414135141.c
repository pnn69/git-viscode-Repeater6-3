      ...
      case modeFanAuxBoxRetro:
        ...
        if (ch != 0xFF) {
          ...
          if (ch == '2') {
            ...
            ESP_LOGI(TAG, "T1   %d Raw", ADC[1]);
            ESP_LOGI(TAG, "T2   %d Raw" ,ADC[2]);
            ESP_LOGI(TAG, "T3   %d Raw",ADC[3]);
            ESP_LOGI(TAG, "T1   %03.02fC ", new_ntc_sample(ADC[1]));
            ESP_LOGI(TAG, "T2   %03.02fC ", new_ntc_sample(ADC[2]));
            ESP_LOGI(TAG, "T3   %03.02fC ", new_ntc_sample(ADC[3]));
          }
