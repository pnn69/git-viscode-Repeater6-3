 ...
 * IO_RS485[DIR][DATA]
 **********************************************************************/
uint8_t IO_RS485[6][2] = {{25, 27}, {14, 4}, {12, 16}, {13, 17}, {15, 5}, {0, 18}}; //{DIR,DATA}
...
void init_UART_RS485(const int uart, uint32_t speed) {
    uart_config_t uart_config = {.baud_rate = speed, .data_bits = UART_DATA_8_BITS, .parity = UART_PARITY_DISABLE, .stop_bits = UART_STOP_BITS_1, .flow_ctrl = UART_HW_FLOWCTRL_DISABLE};
    uart_param_config(uart, &uart_config);
    uart_driver_install(uart, BUF_SIZE * 2, 0, 0, NULL, 0);
}

void init_UART_RS485_hedy(const int uart, uint32_t speed) {
    uart_config_t uart_config = {.baud_rate = speed, .data_bits = UART_DATA_8_BITS, .parity = UART_PARITY_DISABLE, .stop_bits = UART_STOP_BITS_2, .flow_ctrl = UART_HW_FLOWCTRL_DISABLE};
    uart_param_config(uart, &uart_config);
    uart_driver_install(uart, BUF_SIZE * 2, 0, 0, NULL, 0);
}
...
void RS487_humidifier_task(int uart, int port) {
    ...
    while (timeout + pdMS_TO_TICKS(1500) > xTaskGetTickCount()) {
        if (timestamp + pdMS_TO_TICKS(500) < xTaskGetTickCount()) {
            ...
            if (WeifangMuhe.fanspeed < 0x0b)
                WeifangMuhe.fanspeed = 0x0b;
            if (WeifangMuhe.fanspeed > 0x16)
                WeifangMuhe.fanspeed = 0x16;
            writb[0] = (0xaa);
...
void humidifierKabel400(int uart, int port) {
    ...
    if (xSemaphoreTake(xSemaphoreSTR, 100 == pdTRUE)) {
        ...
        if (Kabel400.fanspeed > 0) {  // fan should be turned on
            ...
            } else if (Kabel400.floatsensorError++ > 60) { // increase timeout timer and check if more than 60
                                                           // second are passed
        ...
        if (Kabel400.offTime >= 60 * 60 * 3) {
            ...
            Kabel400.extventOn = false;                     // close external valve
            Kabel400.floatsensorError = 0;                  // reset water low timeout
            if (Kabel400.offTime >= 60 * 60 * 3 + 60 * 5) { // close drain afer 60 sec *60 * 3(3 hours)
                                                            // + 5*60 (and 5 min)
...
void write(const int uart, uint port, char *buf, int len) { uart_write_bytes_RS485(uart, port, buf, len); }
        ...
        if (xSemaphoreTake(xSemaphoreSTR, 10 == pdTRUE)) {
            ...
            if (Fan.speedfan < 4700 / 50 || Fan.speedfan > 4900 / 50) { // Bandfilter fan
                hedy_set_rpm(uart, port,
                             (uint16_t)Fan.speedfan * 50); // convert percentage to rmp 100 = 1Hz
                uart_flush_input(uart);
                f = uart_read_bytes(uart, (uint8_t *)readb, 7, 10);
            }
            xSemaphoreGive(xSemaphoreSTR);
