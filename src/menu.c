...
void OLED_homeScreen() {
    ...
    if (xSemaphoreTake(xSemaphoreSTR, 10 == pdTRUE)) {
        if (mode == modeHumidifier) {
            ...
            sprintf(tbuf, "FanPumpBoxRetro \nFan: %03d%%     \n                \n                  ", (int)(100 - Fanspeed));
        } else
            sprintf(tbuf, "No setup        \nHW:%s           \nSW:%s           \nMEM:%d            ", HW_VERSION, SW_VERSION, NVM_VERSION);
...
void OLED_Show_Version_number(void) {
    sprintf(tbuf,"SW: %s  MEM: %d\n\n\nHW: %s", SW_VERSION, NVM_VERSION, HW_VERSION);
    ssd1306_display_text(tbuf);
...
void setCLEAN(int inp) {
    ...
    if (menuPos[1] == 1 && menuPos[2] == 0) {
        ...
        if (inp == UP) {
            Kabel400.cleanManual = !Kabel400.cleanManual;
        }
        if (inp == DOWN) {
            Kabel400.cleanManual = !Kabel400.cleanManual;
        }
        if (Kabel400.cleanManual) {
            sprintf(tbuf, "CLEAN:AUTO                       ");
        } else {
            sprintf(tbuf, "CLEAN:MANUAL                         ");
        }
        ssd1306_display_text(tbuf);
    }
    if (menuPos[1] == 1 && menuPos[2] == 1) {
...
void setLDR(int inp) {
    ...
    if (Kabel400.ldr == true) {
        ssd1306_display_text("LDR ON      \nON/OFF    ");
    } else {
        ssd1306_display_text("LDR OFF      \nON/OFF    ");
    }
    return;
}
...
void setBeep(int inp) {
    ...
    if (buzzerOnOff == true) {
        ssd1306_display_text("Beep ON      \nON/OFF    ");
    } else {
        ssd1306_display_text("Beep OFF      \nON/OFF    ");
    }
}
