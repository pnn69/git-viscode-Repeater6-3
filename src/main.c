        ...
        case modeHumidifier:
            ...
            if (xSemaphoreTake(xSemaphoreSTR, pdTRUE)) {
                if (Kabel400.fanspeed > 0) {
                    ...
                    // min max check
                    if (tmp < Kabel400.fanspeedminimal)
                        tmp = Kabel400.fanspeedminimal;
                    if (tmp > Kabel400.fanspeedmaximal)
                        tmp = Kabel400.fanspeedmaximal;
                } else {
                    tmp = 0;
                }
            ...
            if (secstamp + OneSec <= xTaskGetTickCount()) {
                ...
                if (xSemaphoreTake(xSemaphoreNTC, pdTRUE)) {
                    if (xSemaphoreTake(xSemaphoreSTR, pdTRUE)) {
                        ...
                        // I:%06.02f",NTC[ToutPos],Fan.speedfan,FanError,NTC[TinPos],NTC[TinPos]-Fan.Tout,Pump.speedpump,PumpError,(CompressorOn
                        // == true) ? "ON":"OFF", Fan.iErrorfan);
